...
自注意力（Self-Attention）是注意力机制的**核心创新**，它让序列中的每个元素都能"看到"序列中的其他所有元素，就像是一个"上下文感知器"：
- 当AI读到"他"这个词时，自注意力会自动找到"他"指代的是谁
- 当AI读到"苹果"这个词时，自注意力会根据上下文判断是指水果还是公司
- 自注意力机制让AI真正拥有了"理解上下文"的能力

我们可以把自注意力比作一个"社交网络分析器"：
- 序列中的每个词都是一个"人"
- 自注意力会计算"人与人之间的关系强度"
- 关系越强的人，越容易被对方"关注"
...
```mermaid
graph TD
    subgraph 输入序列[输入序列<br>社交网络中的人]
        A["I"<br>人A] --> A1[词向量<br>个人特征]
        B["love"<br>人B] --> B1[词向量<br>个人特征]
        C["you"<br>人C] --> C1[词向量<br>个人特征]
    end
    
    subgraph 转换[向量转换<br>分析关系的工具]
        A1 --> D[查询向量<br>我想了解谁?]
        B1 --> D
        C1 --> D
        ...
        A1 --> F[值向量<br>我能提供什么信息?]
        B1 --> F
        C1 --> F
    end
    
    subgraph 注意力计算[注意力计算<br>分析关系强度]
        D --> G[计算注意力分数<br>关系有多强?]
        E --> G
        G --> H[Softmax归一化<br>关系强度归一化]
        H --> I[注意力权重<br>最终关系强度]
        F --> J[加权求和<br>整合关注对象的信息]
        I --> J
    end
    
    subgraph 输出[自注意力输出<br>带有上下文信息的词向量]
        J --> K["I"的上下文向量<br>知道是"爱"的主语]
        J --> L["love"的上下文向量<br>知道主语是"I"，宾语是"you"]
        J --> M["you"的上下文向量<br>知道是"爱"的宾语]
    end
    
    style 输入序列 fill:#e6f3ff,stroke:#333,stroke-width:2px
    style 转换 fill:#fff3e0,stroke:#333,stroke-width:2px
    style 注意力计算 fill:#f0f8ff,stroke:#333,stroke-width:2px
    style 输出 fill:#e8f5e8,stroke:#333,stroke-width:2px
```

**自注意力的优势**：
- **长距离依赖**：无论词之间距离多远，都能建立联系（相当于社交网络中即使是远距离朋友也能被关注）
- **并行计算**：可以同时计算所有词之间的关系（相当于同时分析所有人的社交关系）
- **自适应学习**：自动学习词之间的重要性关系（相当于自动发现谁是重要人物）
